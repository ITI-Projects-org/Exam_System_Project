...
//     // Convert duration from hours to TimeSpan format
//     let durationInHours = formData.duration;
//     if (typeof durationInHours === 'string') {
//       durationInHours = parseFloat(durationInHours);
//     }
    
//     // Convert hours to TimeSpan format (HH:mm:ss)
...
//       Duration: timeSpanString, // Now properly formatted
//       CourseId: parseInt(formData.courseId),
//       MaxDegree: parseInt(formData.maxDegree),
//       MinDegree: parseInt(formData.minDegree),
//       IsAbsent: false,
...
//     // Send directly, don't wrap in examDTO
//     this.ExamTestServices.addExam(examInputDTO).subscribe({
//       next: (response) => {
//         console.log('Exam added successfully:', response);
//         this.router.navigate(['/exams']);
//       },
...
//     this.ExamTestServices.updateExam(examDTO).subscribe({
//       next: (response) => {
...
export class EditExam implements OnInit, OnDestroy {
  ...
  ngOnDestroy(): void {
    if (this.subActive) this.subActive.unsubscribe();
    if (this.getExamSub) this.getExamSub.unsubscribe();
  }
        ...
        this.getExamSub = this.ExamTestServices.getExamById(this.examId).subscribe({
          next: (res) => {
            ...
            this.examForm.patchValue({
              ...
              maxDegree: this.exam.maxDegree ?? 100,
              minDegree: this.exam.minDegree ?? 0,
              courseId: this.exam.courseId ?? 1,
              startDate: this.exam.startDate ? new Date(this.exam.startDate).toISOString().slice(0, 16) : new Date().toISOString().slice(0, 16),
              duration: this.exam.duration ?? 2
            });
  ...
  handel_Edit_Add_Exam(examForm: FormGroup): any {
    ...
    if (this.examId == '0') {
      this.AddExam(examForm);
    } else {
      this.EditExam(examForm, this.examId);
    }
  }
  ...
  EditExam(examForm: FormGroup, examId: string) {
    ...
    this.ExamTestServices.updateExam(examInputDTO).subscribe({
      next: (response) => {
        console.log('Exam updated successfully:', response);
        this.router.navigate(['/exams']);
      },
  ...
  get getTitle() { return this.examForm.controls['title']; }
  get getMaxDegree() { return this.examForm.controls['maxDegree']; }
  get getMinDegree() { return this.examForm.controls['minDegree']; }
  get getStartDate() { return this.examForm.controls['startDate']; }
  get getDuration() { return this.examForm.controls['duration']; }
  get getCourseId() { return this.examForm.controls['courseId']; }
  get getTeacherId() { return this.examForm.controls['teacherId']; }
}